...
def generate_single_year_report(data, feature, year, granularity='Yearly', month=None):
    ...
    try:
        # Additional Section: Display table of loan_status by selected feature
        st.write(f"**Counts of Loan_Status for each {feature} in the selected {granularity} of {year}:**")
        
        # Create a pivot table with the selected feature on the left side and LOAN_STATUS as columns
        loan_status_table = data.groupby([feature, 'LOAN_STATUS']).size().unstack(fill_value=0)
        st.table(loan_status_table)
        ...
        if granularity == 'Yearly':
            ...
            loan_status_counts = year_data['LOAN_STATUS'].value_counts()

            # Display the total counts for each loan status
            st.write(f"**Total counts for Loan_Status in the Year {year}:**")
            ...
            # Calculate total count for each loan status
            loan_status_counts = month_data['LOAN_STATUS'].value_counts()

            # Display the total counts for each loan status
            st.write(f"**Total counts for Loan_Status in {calendar.month_name[month]} {year}:**")
            st.write(loan_status_counts)
            ...
            # Filter data for the selected year and month
            weekly_data = data[(data['year'] == year) & (data['month'] == month)]

            # Calculate total count for each loan status
            loan_status_counts = weekly_data['LOAN_STATUS'].value_counts()

            # Display the total counts for each loan status
            st.write(f"**Total counts for Loan_Status in {calendar.month_name[month]} {year} (Weekly):**")
            st.write(loan_status_counts)
            ...
            # Stacked bar plot
            weekly_data_pivot = weekly_data.groupby(['week', 'LOAN_STATUS'])[feature].count().unstack().fillna(0)
            ...
            # Add numeric annotations on top of the bars
            for i, col in enumerate(weekly_data_pivot.columns):
                for idx, val in enumerate(weekly_data_pivot[col]):
            ...
            daily_data = data[(data['year'] == year) & (data['month'] == month)]

            # Calculate total count for each loan status
            loan_status_counts = daily_data['LOAN_STATUS'].value_counts()

            # Display the total counts for each loan status
            st.write(f"**Total counts for Loan_Status in {calendar.month_name[month]} {year} (Daily):**")
            st.write(loan_status_counts)
            ...
            # Stacked bar plot
            daily_data_pivot = daily_data.groupby(['day', 'LOAN_STATUS'])[feature].count().unstack().fillna(0)
            ...
            # Add numeric annotations on top of the bars
            for i, col in enumerate(daily_data_pivot.columns):
                for idx, val in enumerate(daily_data_pivot[col]):
                    ax.text(idx, val / 2, str(int(val)), color='white', ha='center', va='center')
...
def plot_quarterly_report(data, feature, year, quarter):
    ...
    try:
        ...
        st.write(f"Total counts for Loan_Status in {quarter} of Year {year}:")
        st.write(loan_status_counts)
